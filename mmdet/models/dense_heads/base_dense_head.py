...
from mmdet.core.evaluation import eval_map,tuning_eval_map,eval_recalls
...
class BaseDenseHead(BaseModule, metaclass=ABCMeta):
    ...
    def reward(self,
                ...
                res_baseline=None,):
        """
        res_tuning/res_baseline: bbox_results, results_list:list[tuple[Tensor, Tensor]]
            ...
            gt_bboxes: list
        """
        # sample res_tuning and res_baseline [(n, 5),(n,)]
        # gt_bboxes  (num_gts, 4) gt_labels shape (num_gts,).
        # 思考： 根据res_score>=0.05 筛选出来的结果-》 n>=num_gts
        # print(len(gt_bboxes[0]))
        # for k,v in res_tuning.items():
        #     print(k)
        gt = [{"bboxes":gt_bboxes[i].cpu(),'labels':gt_labels[i].cpu()} for i in range(len(gt_bboxes))]
        # print(len(gt))

        # score_tuning =for x in tuning_eval_map,res_tuning,gt)
        # *eval_recalls(gt_bboxes,res_tuning[1][0]) #取mAP
        score_tuning = torch.tensor(tuning_eval_map(res_tuning,gt))
        # *eval_recalls(gt_bboxes,res_tuning[1][0]) #取mAP
        score_baseline = torch.tensor(tuning_eval_map(res_baseline,gt))
        # score_baseline = multi_apply(tuning_eval_map,res_baseline,gt)
        # *eval_recalls(gt_bboxes,res_baseline[1][0]) #取AR@100
        # batch 平均 ap_score,ar_score
        # print(score_tuning)
        # print(score_baseline)
        return -torch.sum(score_tuning-score_baseline)/len(gt_bboxes)


    def forward_tuning(self,
                      x,
                      img_metas,
                      gt_bboxes,
                      gt_labels=None,
        ...
        tuning_results_list = self.get_bboxes(*outs, img_metas=img_metas)
        tuning_bbox_results = [
            bbox2result(det_bboxes, det_labels, self.num_classes)
            for det_bboxes, det_labels in tuning_results_list
        ]
        res_tuning = tuning_bbox_results
        # print(len(res_tuning[0]))

        reward_loss= self.reward(x,img_metas, gt_bboxes,gt_labels,gt_bboxes_ignore,res_tuning, res_baseline)
        ...
        losses['loss_cls'][-1] *= reward_loss.detach()
        losses['loss_bbox'][-1] *= reward_loss.detach()
